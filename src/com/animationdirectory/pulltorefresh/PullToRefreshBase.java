...
import com.animationdirectory.LogUtils;
import com.animationdirectory.R;
...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout
	...
	public void addView(View child, int index, ViewGroup.LayoutParams params) {
			LogUtils.LOGD(LOG_TAG, "addView: " + child.getClass().getSimpleName());
	...
	public final void setMode(Mode mode) {
		if (mode != mMode) {
			if (DEBUG) {
				LogUtils.LOGD(LOG_TAG, "Setting mode to: " + mode);
			}
			mMode = mode;
			updateUIForMode();
		}
	...
	final void setState(State state, final boolean... params) {
		mState = state;
			LogUtils.LOGD(LOG_TAG, "State: " + mState.name());
	...
	protected final void onSizeChanged(int w, int h, int oldw, int oldh) {
		if (DEBUG) {
			LogUtils.LOGD(LOG_TAG, String.format("onSizeChanged. W: %d, H: %d", w, h));
		}
	...
	protected final void refreshLoadingViewsSize() {
		...
		if (DEBUG) {
			LogUtils.LOGD(LOG_TAG, String.format(
	...
	protected final void setHeaderScroll(int value) {
		if (DEBUG) {
			LogUtils.LOGD(LOG_TAG, "setHeaderScroll: " + value);
		}
